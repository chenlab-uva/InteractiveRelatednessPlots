server <- function(input, output, session) {
  ...
  roh_info_df <- eventReactive(input$EnterFID, {
    ...
    updateSliderInput(session, "F_ROH_X_Range",label = "F_ROH_X_Range",
                      min = min(rohinfo$F_ROH_X), max = max(rohinfo$F_ROH_X), 
                      value = c(min(rohinfo$F_ROH_X),max(rohinfo$F_ROH_X)))
    ...
    if (input$FID!= "All") {
      ...
      updateSelectizeInput(session, "ID", label = "ID", choices = c(Choose=''), selected = NULL)
    }
    return(rohinfo)
  })
  ...
  output$plot3 <- renderPlot({
    ...
    #print(k)
    #write.table(k, row.names = FALSE, quote = FALSE, sep = "\t")
    #validate(need(nrow(k) > 0, "Please select a sample in the study dataset"))
    fid <- k$FID[1]
    id <- k$ID[1]
